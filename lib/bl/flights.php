        ...
        for ($x = 0; $x < $no_of_flights; $x++) {
            if ($values[$ke]["flights"][$x]["bookref"] && $values[$ke]["flights"][$x]["bookref"] === $bookref) {
                if ($values[$ke]["flights"][$x]["flightDate"] && $values[$ke]["flights"][$x]["flightDate"] === $date) {
                    ...
                    $dur = - ((strtotime($post_data[0]["departure"]["scheduledTimeLocal"]) - strtotime($post_data[0]["arrival"]["scheduledTimeLocal"])) / 60);
                    $hour = strval(floor($dur / 60));
                    $min = strval($dur % 60);
                    $values[$ke]["flights"][$x]["duration"] = $hour . "h" . $min . "m";
...
function practice($id, $json_from_file)
{
    ...
    foreach ($list_of_updates as $k => $v) {
        if ($v) {
            ...
            if (!$flight_departs) {
                $data[$k]["departure"] = (object) array(
                    "dep_time" => $list_of_updates[$k]["dep_time"],
            ...
            if (!$flight_arrives) {
                $data[$k]["arrival"] = (object) array(
                    "arr_time" => str_replace(" ", "T", $list_of_updates[$k]["arr_time"]),
